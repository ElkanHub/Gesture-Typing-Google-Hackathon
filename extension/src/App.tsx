...
function App() {
    ...
    const [audioStatus, setAudioStatus] = useState<'idle' | 'generating'>('idle');
    ...
    useEffect(() => {
        ...
        chrome.runtime.onMessage.addListener((message) => {
            ...
            if (message.type === 'AUDIO_STATUS') {
                setAudioStatus(message.status === 'generating' ? 'generating' : 'idle');
            }
        });
    ...
    <div className="flex flex-col h-screen bg-black/50 font-sans selection:bg-indigo-100 overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto p-4 space-y-6 scroll-smooth z-10">
            ...
            {audioStatus === 'generating' && (
                <div className="sticky bottom-4 left-0 right-0 mx-auto w-max max-w-[90%] z-50 animate-in slide-in-from-bottom-2 fade-in duration-300">
                    <div className="bg-indigo-600 text-white px-4 py-2 rounded-full shadow-lg shadow-indigo-200/50 flex items-center gap-2 text-xs font-bold tracking-wide">
                        <span className="w-2 h-2 bg-white rounded-full animate-bounce"></span>
                        <span className="w-2 h-2 bg-white rounded-full animate-bounce [animation-delay:0.1s]"></span>
                        <span className="w-2 h-2 bg-white rounded-full animate-bounce [animation-delay:0.2s]"></span>
                        <span className="ml-1">GENERATING AUDIO...</span>
                    </div>
                </div>
            )}
        </main>
